...
#define TAG "net.devloops.mastercard.gateway"
...
static void TiModule_getBinding(const FunctionCallbackInfo<Value>& args)
{
	...
	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "net.devloops.mastercard.gateway TiModule_getBinding() requires 1 argument: binding");
	...
	if (maybeBinding.IsEmpty()) {
		titanium::JSException::Error(isolate, "net.devloops.mastercard.gateway TiModule_getBinding requires 1 argument: binding. Received argument that could not be converted to a String");
		args.GetReturnValue().Set(scope.Escape(Undefined(isolate)));
		return;
	}
...
static titanium::bindings::BindEntry TiModuleBinding = {
	"net.devloops.mastercard.gateway",
	TiModule_init,
	TiModule_dispose
};
...
Java_net_devloops_mastercard_gateway_TiModuleBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("net.devloops.mastercard.gateway", &TiModuleBinding);
